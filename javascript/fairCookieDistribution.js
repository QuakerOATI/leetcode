...
var distributeCookies = function(cookies, k) {
    var helper = function(children, cookies) {
        if (cookies.length === 0)
        ...
        for (let i=0; i<children.length; ++i) {
            children[i] += c;
            best = Math.min(best, helper(children, cookies));
            children[i] -= c;
        }
        cookies.push(c);
        return best;
    }
    return helper(Array(k).fill(0), cookies.sort());
};

// Wrong answer: greedy algorithm is incorrect here.
var distributeCookiesGreedy = function(cookies, k) {
